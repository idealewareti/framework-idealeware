...
export class SearchComponent implements OnInit {
    ...
    private orderByOptions: Object[] = [
        { label: 'Mais Relevantes' },
        { label: 'Menores Preços', value: '-actualPrice' },
        { label: 'Maiores Preços', value: '+actualPrice' },
            ...
            .subscribe(params => {
                ...
                this.getModule()
                    ...
                    .then(products => {
                        ...
                        this.numPages = Math.ceil((this.results) / this.pageSize);
                        this.createPages();
    ...
    listProducts(page: number, event = null) {
        ...
        if (page <= this.numPages && page >= 1) {
            if (page == 1) {
                cursor = 0
                this.products = this.sortedProducts.slice(cursor, this.pageSize);
            }
            else {
                cursor = (page - 1) * this.pageSize;
                if (this.sortedProducts.length > cursor + this.pageSize)
                    this.products = this.sortedProducts.slice(cursor, this.pageSize + cursor);
                else
                    this.products = this.sortedProducts.slice(cursor);
            }
    ...
    private getProducts(params): Promise<Product[]> {
        return new Promise((resolve, reject) => {
            ...
            this.service.searchFor(this.searchInput, this.page, this.pageSize)
                .then(results => {
                    resolve(results);
                })
                .catch(error => reject(error));
    ...
    changeOrdernation(event) {
        if (this.orderBy && this.orderBy != 'undefined') {
            ...
            this.sortedProducts = this.sortedProducts.sort((a, b) => {
    ...
    resultsFound(): string{
        if(this.results == 1)
            return `${this.results} produto`;
        else if(this.results > 1)
            return `${this.results} produtos`;
        else
