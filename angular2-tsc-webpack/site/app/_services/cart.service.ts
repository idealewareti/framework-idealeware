...
import { ConnectionFactory } from '../_factories/ConnectionFactory';
import { CartDao } from '../_daos/cart.dao';
...
export class CartService {
    ...
    public createCart(cart: Cart): Promise<Cart> {
        return new Promise((resolve, reject) => {
            let item = {
                "product": {
    ...
    public setCustomerToCart(): Promise<Cart> {
        return new Promise((resolve, reject) => {
            if (this.getCartId()) {
                ...
                let url = `${AppSettings.API_CART}/Cart/${this.getCartId()}/Customer`;
