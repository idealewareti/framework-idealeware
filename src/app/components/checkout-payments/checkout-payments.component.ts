...
import { MundipaggPayment } from 'app/models/mundipagg/mundipagg';
...
@Component({
    ...
    templateUrl: '../../views/checkout-payments.component.html',
})
export class CheckoutPaymentsComponent implements OnInit, OnChanges, AfterViewInit {
    ...
    getPaymentNicename(payment: Payment): string{
        return `${AppSettings.getNiceName(payment.name)}${this.isMundipaggBankslip(payment) ? '-bankslip' : ''}-${payment.id}`;
    }
