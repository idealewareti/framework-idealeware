...
@Component({
  selector: 'category',
  templateUrl: '../../../templates/home/category/category.html',
  styleUrls: ['../../../templates/home/category/category.scss']
})
export class CategoryComponent implements OnInit {
  ...
  getPictureCategory(category: Category): string {
    return `${this.store.link}/static/categories/${category.picture}`;
  }

  getRoute(category: Category): string {
    return `/categoria/${AppCore.getNiceName(category.name)}-${category.id}`;
  }

  goRoute(category: Category) {
    if (category.children.length > 0 && this.isMobile()) return null;
    $('.collapse').collapse('hide');
    this.router.navigateByUrl(`/categoria/${AppCore.getNiceName(category.name)}-${category.id}`);
  }
